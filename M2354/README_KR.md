...
Nuvoton M2354 (Cortex-M23) 를 대상으로 하는 AxioCryptoM v1 라이브러리 예제 프로젝트입니다.
...
프로젝트는 M2354BSP 저장소를 아래와 같이 clone한 후 구성해야 합니다.
M2354BSP는 v3.00.005을 사용하여 주십시오.
```bash
git clone https://github.com/OpenNuvoton/M2354BSP.git
git -C M2354BSP checkout v3.00.005
```

> **중요:** `project/Keil/AxioCrypto.uvprojx`는 `../../M2354BSP/..` 경로를 사용합니다.
...
### Flash (총 1 MB : 0x00000000 ~ 0x00100000)
...
| `0x0001A000` ~ `0x00100000` | 920 KB | 애플리케이션 코드 |

### RAM (총 256 KB : 0x20000000 ~ 0x20040000)

| 주소 | 크기 | 영역 설명 |
|------|------|----------|
| `0x20000000` ~ `0x20007000` | 28 KB | 스택 영역 |
| `0x20007000` ~ `0x20007FFF` | 4 KB | **AxioCrypto 데이터 영역** |
| `0x20008000` ~ `0x20040000` | 224 KB | 애플리케이션 데이터 영역 |
...
├── CMSIS/
│   ├── system_M2354.c          # 시스템 초기화
│   └── startup_M2354.s         # 스타트업 코드 / 벡터 테이블
├── User/
│   └── main.c                  # 메인 소스 파일
├── Library/
...
└── AxioCryptoM/
    ├── libaxiocrypto_1.0_m2354_ce.a  # AxioCrypto 라이브러리 (prebuilt)
...
- scatter file `m2354.sct`가 적용되었는지
