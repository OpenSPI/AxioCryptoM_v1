...
This is an AxioCryptoM v1 library example project targeting the Nuvoton M2351 (Cortex-M23).
...
Clone the M2351BSP repository as shown below before setting up the project.

```bash
git clone https://github.com/OpenNuvoton/M2351BSP.git
```

> **Important:** `project/Keil/AxioCrypto.uvprojx` uses the relative path `../../M2351BSP/..`.
...
M2351 total memory capacity:

| Type | Flash | RAM |
|------|-------|-----|
| M2351 Total | 512 KB | 96 KB |

### Flash (Total 512 KB : 0x00000000 ~ 0x00080000)
...
| `0x0001A000` ~ `0x00080000` | 408 KB | Application code |

### RAM (Total 96 KB : 0x20000000 ~ 0x20018000)

| Address | Size | Region |
|---------|------|--------|
| `0x20000000` ~ `0x20007000` | 28 KB | Stack area |
| `0x20007000` ~ `0x20007FFF` | 4 KB | **AxioCrypto data area (reserved)** |
| `0x20008000` ~ `0x20018000` | 64 KB | Application data area |
...
├── CMSIS/
│   ├── system_M2351.c          # System initialization
│   └── startup_M2351.S         # Startup code / Vector table
├── User/
...
└── AxioCrypto/
    ├── libaxiocrypto_1.0_m2351_ce.a    # AxioCrypto library (prebuilt)
...
├── m2351.sct            # Scatter file (memory map settings)
...
- Verify scatter file `m2351.sct` is applied
