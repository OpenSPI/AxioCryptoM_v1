...
typedef enum module_status {
    MODULE_POWER_ONOFF                          = 0,
    MODULE_CRYPTO_INIT                          = 1,
    MODULE_PRE_OPERATIONAL_SELFTEST             = 3,
    MODULE_CONDITIONAL_SELFTEST                 = 4,
    MODULE_APPROVED_MODE                        = 5,
    MODULE_NON_APPROVED_MODE                    = 6,
    MODULE_MODE_CHANGE                          = 7,
    MODULE_MODE_CRITICAL_ERROR                  = 8,
    MODULE_NOTHING                              = 0xFF,
} module_status_t;
...
typedef enum crypto_status {
    ...
    CRYPTO_ERR_NO_SUCH_HANDLE                       = -32,  /* the value of given handle is invalid */
    ...
    CRYPTO_ERR_SYM_CONTEXT_INVALID                  = -206,
    ...
    // DRBG ERR CODE
    CRYPTO_ERR_DRBG_NOT_SUPPORT_ALGORITHM           = -402, /**<  지원하지 않는 DRBG 알고리즘 선택 에러*/
    CRYPTO_ERR_DRBG_NOT_SUPPORT_PR_MODE             = -404, /**< DRBG가 지원하지 않는 PR_MODE 선택 에러 */
    CRYPTO_ERR_DRBG_UPDATE_ENTROPY_INVALID          = -406, /**< 지원하지 않는 DRBG Entropy 갱신 요청 에러 */
    CRYPTO_ERR_DRBG_INIT_PARAMETER_INVALID          = -407, /**< 예측내성이 비활성화 되어, 추가로 필요한 >파라미터가 부족한 경우 */
    CRYPTO_ERR_DRBG_MAXIMUM_REQUEST_BIT_SIZE_OVER   = -408, /**< DRBG가 생성할 수 있는 비트 수를 초과한 경우 */
    CRYPTO_ERR_DRBG_OVER_MAXIMUM_RESEED_INTERVAL    = -409, /**< DRBG에 설정할 수 있는 reseed interval를 >초과한 경우 */
    CRYPTO_ERR_DRBG_REQUEST_BIT_SIZE_INVALID        = -411, /**< DRBG에 요청한 Request Bit가 적절하지 않은 경우(예, Request비트가 0인 경우) */
    CRYPTO_ERR_DRBG_OUT_PARAMETER_INVALID           = -412, /**< DRBG가 생성한 랜덤한 수가 저장되어야 하는 변수와 변수비트사이즈가 올바르지 않은 경우(예, 변수가 NULL이거나 변수비트사이즈가 0인경우) */
    CRYPTO_ERR_DRBG_HEALTHTEST_FAIL                 = -413, /**< DRBG test fail */
    CRYPTO_ERR_TRNG_HEALTHTEST_FAIL                 = -490, /**<  @~korean TRNG health test 실패 */
    CRYPTO_ERR_PBKDF_SHORT_LENGTH                   = -801, /**< iter count or salt length is too short */

        // AxioCrypto-M error code
    CRYPTO_ERR_PLATFORM_CHECK_BINARY_FAIL           = -1000, /**< @~korean 플랫폼 바이너리 체크 실패 @~english */
    CRYPTO_ERR_PLATFORM_INIT_FAIL                   = -1002, /**< @~korean 플랫폼 초기화 실패 @~english */
    CRYPTO_ERR_VALIDATE_OVER_COUNT                  = -1100, /**< @~korean API 사용 횟수 초과 @~english */
    CRYPTO_ERR_VALIDATE_OVER_SLOT                   = -1101, /**< @~korean API 사용 슬롯 초과 @~english */
    CRYPTO_ERR_INTEGRITY_CTX_FAIL                   = -1110, /**< @~korean 무결성 검증 실패 @~english */
    CRYPTO_ERR_INTEGRITY_CTX_UPDATE_FAIL            = -1111, /**< @~korean 무결성 검증 컨텍스트 갱신 실패 @~english */   
    CRYPTO_ERR_INTEGRITY_HEAP_FAIL                  = -1112, /**< @~korean 무결성 검증 힙 영역 실패 @~english */
    CRYPTO_ERR_INTEGRITY_HEAP_UPDATE_FAIL           = -1113, /**< @~korean 무결성 검증 힙 영역 갱신 실패 @~english */
	CRYPTO_ERR_ACTIVATION_CODE_INVALID              = -1200, /**< @~korean 활성화 코드가 유효하지 않음 @~english */
    CRYPTO_ERR_ACTIVATION_CODE_NOT_SUPPORTED        = -1201, /**< @~korean 활성화 지원하지 않는 제품 @~english */
    CRYPTO_ERR_BUSY                                 = -1300, /**< @~korean 다른 작업이 진행 중임 @~english */
} CRYPTO_STATUS;
...
/**
 * @~korean  @brief  HASH 상태 데이터 복제
 * @~korean  @param  src_handle  [in]  원본 컨텍스트가 저장된 슬롯의 번호
 * @~korean  @param  dst_handle  [in]  복제 될 컨텍스트가 저장된 슬롯의 번호
 * @~korean  @return
 *      @li <code>CRYPTO_SUCCESS</code>: 성공
 * @li <code>CRYPTO_ERR_HASH_CTX_HANDLE_INVALID</code>: 주어진 handle이 유효하지 않음.
 * @li <code>CRYPTO_ERR_HASH_CTX_NOT_OPEN</code>: <code>axiocrypto_hash_init()</code>을 실행하지 않았음.
 *
 * @~english @brief  This function replicates hash algorithm state data.
 * @~english @param  src_handle  [in]  source context handle
 * @~english @param  dst_handle  [in]  destination context handle
 * @~english @return
 *      @li <code>CRYPTO_SUCCESS</code>: the function returned OK
 * @li <code>CRYPTO_ERR_HASH_CTX_HANDLE_INVALID</code>: <code>handle</code> is not valid
 * @li <code>CRYPTO_ERR_HASH_CTX_NOT_OPEN</code>: <code>axiocrypto_hash_init()</code> was not called
...
CRYPTO_STATUS axiocrypto_ecdh_getkey(const ctx_handle_t slot, uint8_t *KT, const uint32_t KTsz);
/**
 * @}
 */
...
typedef struct{
    int log_status;                 // 0: disabled, 1: enabled  
    int integrity_check_status;     // 0: disabled, 1: enabled
    uint32_t api_call_count;        // Number of API calls
    int activation_status;          // 0: not activated, 1: activated
    char crypto_ver[128];           // Crypto Version
    char library_ver[128];          // Library Version
    uint8_t digest[64];             // Crypto Digest
    char lic_type[64];              // License Type
    char target[64];                // Target
    uint8_t uid[16];                // Unique Identifier

}axiocrypto_status_t;

CRYPTO_STATUS axiocrypto_status(axiocrypto_status_t *status);

CRYPTO_STATUS axiocrypto_activate(char *cid, uint8_t code[32]);

CRYPTO_STATUS axiocrypto_enable_log();

CRYPTO_STATUS axiocrypto_disable_log();

// Not yet implemented
CRYPTO_STATUS axiocrypto_enable_integrity_check();

// Not yet implemented
CRYPTO_STATUS axiocrypto_disable_integrity_check();
